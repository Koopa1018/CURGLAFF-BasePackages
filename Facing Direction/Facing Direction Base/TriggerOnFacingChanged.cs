...
{
	public class TriggerOnFacingChanged : MonoBehaviour {
		/// <summary>
		...
		[SerializeReference]
#endif
		IFacingDirection facingDirectionToCheck;
		...
		void Awake() {
			if (facingDirectionToCheck == null) {
				facingDirectionToCheck = GetComponent<IFacingDirection>();
			}
#if UNITY_EDITOR
			...
			lastFacingDirection = facingDirectionToCheck.normalized();
		}
